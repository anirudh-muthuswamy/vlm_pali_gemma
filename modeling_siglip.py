...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        patch_embeds = self.patch_embedding(pixel_values)  # [batch_size, embed_dim, num_patches_height, num_patches_width]
        ...
        embeddings += self.position_embedding(self.position_ids)  # Add position embeddings
        return embeddings
...
class SiglipAttention(nn.Module):
    def __init__(self, config:SiglipVisionConfig):
        ...
        self.dropoout = config.attention_dropout
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if attn_weights.size() != (batch_size, self.num_heads, seq_length, seq_length):
            raise ValueError(
                ...
                f"size: {attn_weights.size()}"
            )
        #apply softmax row-wise. attn_weights: [batch_size, num_heads, num_patches, num_patches]
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        #Apply dropout only during training
        attn_weights = nn.functional.dropout(attn_weights, p = self.dropoout, training=self.training)
        #multiply attn weights by value states [batch_size, num_heads, num_patches, head_dim]
