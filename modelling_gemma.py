...
class KVCache():
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            # each tensor has the shape: [batch_size, num_heads_kv, seq_len, head_dim]
...
#converts the hidden dimension of the vision model to the projection dimension which is equal
# to the embedding size of the text
...
def repeat_kv(hidden_states: torch.Tensor, n_rep:int) -> torch.Tensor:
    batch, num_key_value_heads, s_len, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, None, :, :].expand(batch, num_key_value_heads, n_rep, s_len, head_dim)
    return hidden_states.reshape(batch, num_key_value_heads*n_rep, s_len, head_dim)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        #(batch_size, seq_len, num_heads_k_v * head_dim)
        key_states = self.k_proj(hidden_states)
        #(batch_size, seq_len, num_heads_k_v * head_dim)
        value_states = self.v_proj(hidden_states)
        #(batch_size, num_heads_q, seq_len, head_dim)
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1,2)
        #(batch_size, num_heads_k_v, seq_len, head_dim)
        ...
        #repeat the key and valueto match the number of heads of the query
        ...
        #multiply by w_o which will mix all the dimensions which each other. So that each head's
        #result is mixed with each other through the w_o projection
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(self, 
        ...
        #combine the embeddings of the image_tokens, the text tokens and mask out all the padding tokens
        final_embedding = torch.zeros(batch_size, sequence_length, embed_dim, dtype=input_embeds.dtype,
        ...
        if kv_cache is None or kv_cache.num_items() == 0:
            ...
            #generating tokens, query is one single token
            assert q_len==1
